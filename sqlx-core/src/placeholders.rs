...
use crate::arguments::{ArgumentIndex, PositionalArguments};
...
// aka parsed arguments
pub struct Arguments<DB: Database> {
    _phantom: PhantomData<DB>,
}

impl<DB: Database> Arguments<DB> {
    fn get(&self, _idx: &ArgumentIndex<'_>) -> Option<&FooArgument> {
        todo!()
    }
}

struct FooArgument {
    value_vector_len: Option<usize>,
}
...
impl Placeholder<'_> {
    #[allow(unused)]
    pub(crate) fn into_static(self) -> Placeholder<'static> {
        Placeholder {
            token: self.token,
...
impl Ident<'_> {
    ...
    pub(crate) fn into_static(self) -> Ident<'static> {
        match self {
            Ident::Implicit => Ident::Implicit,
            Ident::Positional(pos) => Ident::Positional(pos),
            Ident::Named(n) => Ident::Named(n.to_string().into()),
            Ident::Field(f) => Ident::Field(f.to_string().into()),
        }
    }
}
...
impl<'a> ParsedQuery<'a> {
    ...
    pub fn expand<
        DB: Database,
        FnArgKind: FnMut(&ArgumentIndex<'_>, &Placeholder<'a>) -> Result<ArgumentKind, String>,
        PosArgs: PositionalArguments<'static>,
        PosArgsCtor: FnOnce() -> PosArgs,
    >(
        &self,
        get_arg_kind: FnArgKind,
        pos_args_ctor: PosArgsCtor,
    ) -> Result<(Cow<'a, str>, PosArgs)> {
        self.expand_inner(
            DB::PLACEHOLDER_CHAR,
            DB::PARAM_INDEXING,
            get_arg_kind,
    ...
    fn expand_inner<PosArgs: PositionalArguments<'static>, PosArgsCtor: FnOnce() -> PosArgs>(
        &self,
        placeholder_char: char,
        indexing: ParamIndexing,
        mut get_arg_kind: impl FnMut(
            &ArgumentIndex<'_>,
            &Placeholder<'a>,
        ...
        if self.placeholders.is_empty() {
            let sql = self.query.into();
            let args_buffer = pos_args_ctor();

            return Ok((sql, args_buffer));
        }
        ...
        for placeholder in &self.placeholders {
            ...
            match get_arg_kind(&argument, placeholder).map_err(|e| {
                Error::PlaceholderToArgument {
...
/// is necessary.
#[allow(unused)]
fn default_get_arg<DB: Database>(
    args: &Arguments<DB>,
) -> impl FnMut(&ArgumentIndex<'_>, &Placeholder<'_>) -> Result<ArgumentKind, String> + use<'_, DB>
{
    move |idx, _place| {
...
fn test_expand_parsed_query() -> Result<()> {
    ...
    impl<'q> PositionalArguments<'q> for NoopPosArgs {
        type Database = Any;
        ...
        fn add<T>(&mut self, _value: T) -> std::result::Result<(), BoxDynError>
        where
            T: 'q + Encode<'q, Self::Database> + Type<Self::Database> + Send + Sync,
        {
            todo!()
        }
    ...
    // Postgres
    assert_eq!(
        parse_query("SELECT 1 FROM foo WHERE bar = {} AND baz = {baz}")?
            .expand_inner(
                '$',
                OneIndexed,
                args! {
